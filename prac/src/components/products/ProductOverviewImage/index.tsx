            ...
            {productImagesData?.map((item, index) => {
              return (
                <SwiperSlide
                  ...
                  key={item.id}
                  className={clsx(
                    "bg-white box-border border-2 border-gray/20 inline-block relative aspect-square hover:cursor-pointer select-none",
                    { ["border-greenDark-50"]: currentIndex === index }
                  )}
                >
                  <Image
                    src={item.url}
